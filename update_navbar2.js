...
const filePath = path.join(__dirname, 'src/_layouts/Navbar.tsx');
let content = fs.readFileSync(filePath, 'utf8');

const navOld = `<nav className={\`shadow-[0_4px_12px_rgba(0,0,0,0.6)] border-b-4 border-amber-900 fixed top-0 left-0 w-full z-50 \${theme === 'dark' ? 'bg-[#1a0f0a]' : 'bg-[#3e2723]'}\`}>`;
const navNew = `<nav className={\`backdrop-blur-md border-b border-amber-500/20 fixed top-0 left-0 w-full z-50 transition-all duration-300 \${theme === 'dark' ? 'bg-[#0b0604]/80 shadow-[0_8px_32px_rgba(0,0,0,0.8)]' : 'bg-[#3e2723]/90 shadow-lg'}\`}>`;

const div1Old = `<div className="w-full px-4 sm:px-6 lg:px-8">`;
const div1New = `<div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">`;

const div2Old = `<div className="flex h-14 items-center justify-between gap-4">`;
const div2New = `<div className="flex h-16 items-center justify-between gap-6">`;

const logoOld = `<Link href="/" className="flex shrink-0 items-center gap-2 group" data-tour="navbar-logo">
                <ImageWithFallback
                  src={APP.image_logo}
...
const logoNew = `<Link href="/" className="flex shrink-0 items-center gap-3 group" data-tour="navbar-logo">
                <ImageWithFallback
                  src={APP.image_logo}
                  alt={\`\${APP.shortName} logo\`}
                  size={44}
...
const profileNew = `<Link href="/profile" className="flex items-center gap-2.5 px-1.5 py-1 rounded-full border border-transparent hover:border-amber-500/30 hover:bg-amber-900/20 transition-all duration-300 group" data-tour="navbar-profile">
...
content = content.replace(div1Old, div1New);
content = content.replace(div2Old, div2New);
content = content.replace(logoOld, logoNew);

// Replace all instances of linkOld1 and linkOld2 with linkNew
content = content.split(linkOld1).join(linkNew);
content = content.split(linkOld2).join(linkNew);

content = content.replace(attackOld, attackNew);
content = content.replace(hofOld, hofNew);
