...
export default function MonitoringList({ solves, isLoading, onRefresh }: MonitoringListProps) {
  ...
  if (solves.length === 0) {
    return (
      <div className="text-center py-16 border-2 border-dashed border-amber-900/50 dark:border-gray-700 rounded-sm">
    ...
    <div className="space-y-4">
      {solves.map((s) => {
        ...
        return (
          <Card
            key={s.solve_id}
            className={`transition-all duration-300 relative overflow-hidden bg-[#fdf6e3] dark:bg-[#1A100C] border ${
              isSuspicious
                ? 'border-red-500/40 shadow-[0_0_15px_rgba(239,68,68,0.05)] dark:border-red-900/40 dark:shadow-[0_0_20px_rgba(239,68,68,0.1)]'
                : 'border-amber-900/50 dark:border-gray-700 shadow-[0_4px_12px_rgba(0,0,0,0.6)] hover:border-gray-300 dark:hover:border-gray-600'
            }`}
          >
            {/* Alert Indicator Bar */}
            {isSuspicious && (
            ...
            <CardContent className={`p-4 sm:p-5 ${isSuspicious || isFlawlessOneshot ? 'pl-6 sm:pl-7' : ''}`}>
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                ...
                <div className="space-y-2 max-w-xl">
                  <div className="flex flex-wrap items-center gap-2">
                    <Link
                      ...
                      className="font-bold text-base text-amber-700 dark:text-amber-500 dark:text-blue-400 hover:underline flex items-center gap-1"
                    >
                    ...
                    {s.team_name && s.team_name.trim() !== '' && (
                      <span className="text-xs px-2 py-0.5 rounded bg-[#eaddb6] dark:bg-gray-700 text-gray-600 dark:text-gray-300 font-mono font-semibold">
                        [ {s.team_name} ]
                      </span>
                    )}
                  ...
                  <div className="flex flex-col gap-2 pt-1">
                    {isFlagSharing && (
                      <div className="flex items-start gap-2 text-xs font-semibold text-red-600 dark:text-red-400 bg-red-500/10 dark:bg-red-950/30 px-3 py-2 rounded-sm border border-red-500/20 max-w-xl">
                    ...
                    {isAiSuspicious && (
                      <div className="flex flex-col gap-2 p-3 bg-red-500/5 dark:bg-red-950/10 border border-red-500/20 rounded-sm max-w-xl">
                ...
                <div className="flex flex-wrap lg:flex-col lg:items-end gap-3 justify-start lg:justify-center border-t lg:border-t-0 border-amber-900/30 dark:border-gray-700/60 pt-3 lg:pt-0">
                  {/* Solve Badges */}
                  <div className="flex items-center gap-2">
                    {isFlawlessOneshot ? (
                      <span className="inline-flex items-center gap-1 text-xs font-black bg-gradient-to-r from-yellow-400 to-amber-500 text-black px-2.5 py-1 rounded shadow-[0_4px_12px_rgba(0,0,0,0.6)] border border-yellow-300">
                        <Award className="w-3 h-3" />
                        âš¡ ONESHOT
                      </span>
                    ) : isOneshot ? (
                      <span className="inline-flex items-center gap-1 text-xs font-bold bg-amber-500 text-black px-2.5 py-1 rounded shadow-[0_4px_12px_rgba(0,0,0,0.6)]">
                        <Award className="w-3 h-3" />
                        ONESHOT
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-xs font-semibold bg-[#eaddb6] dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded">
                        Attempts: {s.incorrect_attempts_count + 1}
                      </span>
                    )}
                    
                    {isSuspicious ? (
                      <span className="inline-flex items-center gap-1 text-xs font-bold bg-red-600 text-white px-2.5 py-1 rounded shadow-[0_6px_16px_rgba(0,0,0,0.8)] animate-pulse">
                        <ShieldAlert className="w-3 h-3" />
                        SUSPICIOUS
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-xs font-bold bg-emerald-600 text-white px-2.5 py-1 rounded shadow-[0_4px_12px_rgba(0,0,0,0.6)]">
