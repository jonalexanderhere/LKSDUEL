          ...
          <div className="flex h-14 items-center justify-between gap-4">
            ...
            <div className="flex min-w-0 flex-1 items-center space-x-4 2xl:space-x-6">
              <Link href="/" className="flex shrink-0 items-center gap-2 group" data-tour="navbar-logo">
                ...
                />
                <span className={`text-[1.35rem] font-extrabold tracking-wide ${theme === 'dark' ? 'text-white' : 'text-amber-100'} transition-all duration-200 group-hover:text-amber- dark:group-hover:text-amber-`}>{APP.shortName}</span>
              </Link>
            ...
            <div className="flex shrink-0 items-center gap-2 2xl:gap-4">
              ...
              {user && (
                <div className="relative mr-2" data-tour="navbar-logs">
                  <button
                    className={`rounded-full p-1 transition-colors duration-150 ${pathname === '/logs' ? (theme === 'dark' ? 'bg-amber-' : 'bg-amber-') : ''}`}
                    title="Logs"
                    aria-label="Logs"
                    onClick={() => {
                      if (pathname === '/logs') {
              ...
              {process.env.NODE_ENV === 'development' && (
                <button
                  ...
                  className="p-1 rounded-full hover:bg-amber- dark:hover:bg-amber-/30 transition-colors mr-1"
                  title="Platform Setup (Dev Only)"
                >
                  <Settings2 size={22} className="text-amber-" />
                </button>
              )}
              ...
              <div className="px-4 pt-4 pb-6 space-y-2 animate-fade-in">
                ...
                {user && (
                  ...
                  >
                    ...
                    <span
                      className={`text-[15px] font-semibold ${theme === 'dark' ? 'text-white' : 'text-amber-100'} group-hover:text-amber- dark:group-hover:text-amber- truncate whitespace-nowrap max-w-[120px] block`}
                      title={user.username}
                    >
                      {user.username}
                    </span>
