    ...
    <div className="space-y-3">
      ...
      <AnimatePresence>
        {featuredFirstBlood && energyPhase && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm pointer-events-none">
            <motion.div
              ...
              animate={{ 
                ...
                scale: 1, 
                rotate: 0,
                x: [0, -0.6, 0.6, -0.4, 0.4, -0.6, 0.6, 0],
                y: [0, 0.6, -0.6, 0.4, -0.4, 0.6, -0.6, 0]
              }}
              exit={{ opacity: 0, filter: 'blur(10px)' }}
              transition={{ 
                x: { repeat: Infinity, duration: 0.18, ease: "linear" },
                y: { repeat: Infinity, duration: 0.18, ease: "linear" },
                default: { duration: 0.45, ease: "easeOut" }
              }}
              className="relative w-full max-w-lg overflow-hidden rounded-xl border-2 border-red-900 bg-black px-5 py-20 shadow-[0_0_80px_rgba(220,38,38,0.75)]"
            >
        ...
        {featuredFirstBlood && showFeaturedFirstBlood && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md">
            <motion.div
              ...
              transition={{ 
                x: { repeat: Infinity, duration: 0.22, ease: "linear" },
                y: { repeat: Infinity, duration: 0.22, ease: "linear" },
                default: { type: 'spring', stiffness: 180, damping: 22, mass: 0.8 }
              }}
              className="relative w-full max-w-xl overflow-hidden rounded-xl border-2 border-red-800 bg-black px-5 py-12 shadow-[0_0_60px_rgba(220,38,38,0.55)]"
            >
              {/* Manual Close Button for premium UX control */}
              <button 
                onClick={() => setShowFeaturedFirstBlood(false)}
                className="absolute top-4 right-4 z-30 text-red-500 hover:text-white transition-colors text-2xl font-bold bg-black/40 hover:bg-red-950/40 w-8 h-8 rounded-full flex items-center justify-center pointer-events-auto"
                title="Close"
              >
                &times;
              </button>
              ...
              <div className="relative z-10 flex flex-col items-center justify-center pt-16">
                ...
                {/* Challenge Category Text - Fully integrated cleanly below title */}
                <div className="mt-3 text-xs font-mono font-bold tracking-[0.2em] text-rose-500 uppercase bg-red-950/40 border border-red-900/60 rounded px-2.5 py-0.5">
